...
const fetchArticleDetail = async () => {
  try {
    ...
    if (res.data.code === 0 && res.data.data) {
      ...
      formState.author = res.data.data.author || '未填写作者'
      formState.title = res.data.data.title || '未填写标题'
      coverImage.value = res.data.data.coverImage || ''
      if (res.data.data.summary) {
        res.data.data.summary.forEach((item: any) => {
          formState.description += item + '\n'
        })
      }
    } else {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const params = ref<API.ArticleUpdateRequest>({
      id: props.id,
    ...
    const res = await updateArticleUsingPost(params.value)
    if (res.data.code === 0) {
